// ======================================================
// BOTÓN IZQUIERDA
// ======================================================

const botonIzquierda =
    document.querySelector(
        "#btn-izquierda"
    );


botonIzquierda.addEventListener(
    "click",
    function () {

        // Primero hay que seleccionar una ficha
        if (
            fichaSeleccionada ===
            null
        ) {

            mostrarMensaje(
                "Selecciona una ficha antes de elegir un lado."
            );


            return;
        }


        const jugadaCorrecta =
            jugarFichaJugador(
                fichaSeleccionada,
                "izquierda"
            );


        // ==================================================
        // JUGADA CORRECTA
        // ==================================================

        if (jugadaCorrecta) {

            fichaSeleccionada =
                null;


            actualizarInterfazVisual();


            // Si el jugador acaba de ganar
            if (
                partidaTerminada
            ) {

                mostrarMensajeResultado();


                return;
            }


            /*
            Si después de jugar sigue
            siendo turno del jugador,
            significa que ha coincidido
            número + color.

            Puede ser:

            - bloqueo normal
            - bloqueo + roba 2
            */

            if (
                turno ===
                "jugador"
            ) {

                mostrarMensajeEfecto();


                activarControlesJugador();


                return;
            }


            /*
            Si no hubo coincidencia
            de color, ahora juega
            la máquina.
            */

            if (
                turno ===
                "maquina"
            ) {

                ejecutarTurnoMaquinaVisual();
            }


        // ==================================================
        // JUGADA INCORRECTA
        // ==================================================

        } else {

            mostrarMensaje(
                "Esa ficha no encaja en el extremo izquierdo."
            );
        }
    }
);


// ======================================================
// BOTÓN DERECHA
// ======================================================

const botonDerecha =
    document.querySelector(
        "#btn-derecha"
    );


botonDerecha.addEventListener(
    "click",
    function () {

        // Primero hay que seleccionar una ficha
        if (
            fichaSeleccionada ===
            null
        ) {

            mostrarMensaje(
                "Selecciona una ficha antes de elegir un lado."
            );


            return;
        }


        const jugadaCorrecta =
            jugarFichaJugador(
                fichaSeleccionada,
                "derecha"
            );


        // ==================================================
        // JUGADA CORRECTA
        // ==================================================

        if (jugadaCorrecta) {

            fichaSeleccionada =
                null;


            actualizarInterfazVisual();


            // Si el jugador acaba de ganar
            if (
                partidaTerminada
            ) {

                mostrarMensajeResultado();


                return;
            }


            /*
            Si continúa siendo turno
            del jugador, ha conseguido
            una coincidencia de color.
            */

            if (
                turno ===
                "jugador"
            ) {

                mostrarMensajeEfecto();


                activarControlesJugador();


                return;
            }


            /*
            Si no hubo efecto,
            responde la máquina.
            */

            if (
                turno ===
                "maquina"
            ) {

                ejecutarTurnoMaquinaVisual();
            }


        // ==================================================
        // JUGADA INCORRECTA
        // ==================================================

        } else {

            mostrarMensaje(
                "Esa ficha no encaja en el extremo derecho."
            );
        }
    }
);


// ======================================================
// BOTÓN ROBAR
// ======================================================

const botonRobar =
    document.querySelector(
        "#btn-robar"
    );


botonRobar.addEventListener(
    "click",
    function () {

        const roboCorrecto =
            robarFichaJugador();


        // ==================================================
        // ROBO CORRECTO
        // ==================================================

        if (roboCorrecto) {

            fichaSeleccionada =
                null;


            actualizarInterfazVisual();


            const jugablesJugador =
                obtenerFichasJugables(
                    fichasJugador
                );


            /*
            El jugador sigue robando
            hasta conseguir una ficha
            jugable o vaciar el pozo.
            */

            if (
                jugablesJugador.length >
                0
            ) {

                mostrarMensaje(
                    "Has robado una ficha jugable. Selecciónala y juega."
                );


            } else if (
                fichasPozo.length >
                0
            ) {

                mostrarMensaje(
                    "La ficha robada no sirve. Debes volver a robar."
                );


            } else {

                mostrarMensaje(
                    "La ficha robada no sirve y el pozo está vacío. Debes pasar."
                );
            }


        // ==================================================
        // NO PUEDE ROBAR
        // ==================================================

        } else {

            // No es su turno
            if (
                turno !==
                "jugador"
            ) {

                mostrarMensaje(
                    "Espera a que termine el turno de la máquina."
                );


            // Tiene alguna ficha jugable
            } else if (
                obtenerFichasJugables(
                    fichasJugador
                ).length > 0
            ) {

                mostrarMensaje(
                    "Tienes una ficha jugable. No puedes robar."
                );


            // El pozo está vacío
            } else if (
                fichasPozo.length ===
                0
            ) {

                mostrarMensaje(
                    "El pozo está vacío. Debes pasar el turno."
                );


            } else {

                mostrarMensaje(
                    "No puedes robar en este momento."
                );
            }
        }
    }
);


// ======================================================
// BOTÓN PASAR
// ======================================================

const botonPasar =
    document.querySelector(
        "#btn-pasar"
    );


botonPasar.addEventListener(
    "click",
    function () {

        const paseCorrecto =
            pasarTurnoJugador();


        // ==================================================
        // PUEDE PASAR
        // ==================================================

        if (paseCorrecto) {

            fichaSeleccionada =
                null;


            actualizarInterfazVisual();


            /*
            Al pasar puede haberse
            detectado un final
            por bloqueo.
            */

            if (
                partidaTerminada
            ) {

                mostrarMensajeResultado();


                return;
            }


            mostrarMensaje(
                "Has pasado el turno."
            );


            if (
                turno ===
                "maquina"
            ) {

                ejecutarTurnoMaquinaVisual();
            }


        // ==================================================
        // NO PUEDE PASAR
        // ==================================================

        } else {

            // No es su turno
            if (
                turno !==
                "jugador"
            ) {

                mostrarMensaje(
                    "Espera a que termine el turno de la máquina."
                );


            // Tiene una ficha jugable
            } else if (
                obtenerFichasJugables(
                    fichasJugador
                ).length > 0
            ) {

                mostrarMensaje(
                    "Tienes una ficha jugable. No puedes pasar."
                );


            // Todavía quedan fichas
            // en el pozo
            } else if (
                fichasPozo.length >
                0
            ) {

                mostrarMensaje(
                    "No puedes pasar todavía. Debes robar una ficha."
                );


            } else {

                mostrarMensaje(
                    "No puedes pasar en este momento."
                );
            }
        }
    }
);


// ======================================================
// COMPROBAR QUIÉN DEBE JUGAR AL INICIAR
// ======================================================

if (
    turno === "maquina" &&
    !partidaTerminada
) {

    ejecutarTurnoMaquinaVisual();


} else {

    mostrarMensaje(
        "Tu turno. Selecciona una ficha."
    );
}


// ======================================================
// BOTÓN NUEVA PARTIDA
// ======================================================

const botonNuevaPartida =
    document.querySelector(
        "#btn-nueva-partida"
    );


botonNuevaPartida.addEventListener(
    "click",
    function () {

        location.reload();
    }
);